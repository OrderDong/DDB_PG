    ...
    function _getAuctionByType(uint8 _eType,address _owner) view internal returns (uint256) {
        ...
        for(i=0;i<auctionByTokenId[_owner].length;i++){
            if(auctions[auctionByTokenId[_owner][i]].eType == _eType){
                count.add(auctions[auctionByTokenId[_owner][i]].salesCount);
                break;
            }
        }
        return count;
...
contract EggAuctionCore is EggAuction{
    ...
    function exchangeEggToDDC(uint256 _auctionId,uint256 _count,uint8 _eType,uint8 _level) public payable whenNotPaused {
        uint256 eggCount = _getEggCount(msg.sender,_eType);
    ...
    function exchangeDDCToEgg(uint256 _count,uint8 _eType,uint8 _level) public payable whenNotPaused {
        require(_count > 0);
