...
contract Market20 is Ownable,Pausable {
    ...
    function _getEggCount(address _owner,uint8 _eType) internal view returns (uint256){
        uint256 i;
        uint256 count;
        if(mappEggMk[_owner].length >0 ){
            for(i=0;i<mappEggMk[_owner].length;i++){
                if(eggs[mappEggMk[_owner][i]].eType == _eType){
                    count.add(eggs[mappEggMk[_owner][i]].mkCount);
                }
            }
        }
        return count;
...
contract SalesAndPlay is MarketUse {
    struct Auction {
    ...
    function _getAuctionCountByType(uint8 _eType) view internal returns (uint256) {
        uint256 i;
        uint256 count;
        for(i=0;i<auctions.length;i++){
            if(auctions[i].eType == _eType){
                count.add(auctions[i].salesCount);
            }
        }
        return count;
    }

    function createEggAuction(uint256 priceInWei, uint256 expiresAt,uint256 _count,uint8 _eType,uint8 _level) public whenNotPaused {
        ...
        //判断owner拥有数量大于托管数量
        uint256 ownerCount = _getEggCount(msg.sender,_eType);
        uint256 auctionCount = _getAuctionCountByType(_eType).add(_count);
        require( ownerCount>= auctionCount);
    ...
    function executeEggAuction(uint256 _auctionId,uint256 _count) public payable whenNotPaused {
        ...
        //transfer DDC
        //require(acceptedToken.transferFrom(owner,msg.sender,publicationFeeInWei.mul(_count)));
        // Transfer token owner
        _transferEggMk(owner,msg.sender,_count,eType,0);
        uint256 i = _getAuctionIndex(_auctionId,msg.sender);
